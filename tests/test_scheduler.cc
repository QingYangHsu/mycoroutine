...
void test_fiber2() {
     ...
     */
    //std::cout<<"tag 6-1"<<std::endl;
    //sleep(3);             //这里暂时不支持 原因还未查明
   // std::cout<<"tag 6-2"<<std::endl;
...
int main() {
  //  std::cout<<"tag0"<<std::endl;
     ...
     * 消耗掉，然后再返回main函数往下执行
     */
    sylar::Scheduler sc;  //只调度caller线程
    //sylar::Scheduler sc(2); //调度caller线程和另一个工作线程
    // sylar::Scheduler sc(2, false); //不调度caller线程，只调度一个工作线程

    // 额外创建新的线程进行调度，那只要添加了调度任务，调度器马上就可以调度该任务
    // sylar::Scheduler sc(3, false);

    // 主线程添加调度任务，使用函数作为调度对象
  ...
  //  std::cout<<"tag1"<<std::endl;
   ...
   // std::cout<<"tag2"<<std::endl;
    ...
    // 创建调度线程(工作线程)，开始任务调度，如果只使用main函数线程进行调度，那start相当于什么也没做
    sc.start();
...
//g++ test_scheduler.cc ../src/fiber.cc ../src/scheduler.cc ../src/util.cpp ../src/thread.cc ../src/mutex.cc ../src/iomanager.cc ../src/timer.cc -o test -std=c++11 -lpthread