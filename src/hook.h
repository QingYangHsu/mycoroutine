...
namespace sylar {
    /**
     * @brief 当前线程是否hook，我们在hook.cc定义了一个线程局部变量标定该线程是否hook，如果启用hook我们就可以在系统调用上附加自己的操作
     */
    bool is_hook_enable();
    /**
     * @brief 设置当前线程的hook状态
...
//sleep
//声明了一个函数指针类型sleep_fun
typedef unsigned int (*sleep_fun)(unsigned int seconds);

//定义了一个函数指针sleep_f，默认初始化为NULL
...
//实现带超时的connect，这个函数并不是系统默认的，所以需要自己实现
