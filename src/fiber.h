...
public:
    ...
    enum State {
        ...
        /// 运行态，resume之后的状态
        RUNNING,
        /// 结束态，协程的回调函数执行完之后为TERM状态 该状态改变在MAINFUNC中
        TERM
    };
...
public:
     ...
     * @param[in] run_in_scheduler 本协程是否参与调度器调度，默认为true 即接收调度
     */
...
public:
     ...
     * 且该协程为当前线程的主协程，其他协程都通过这个协程来调度，也就是说，其他协程结束时,都要切回到主协程，由主协程重新选择新的协程进行resume
...
private:
    ...
    /// 本协程是否参与调度器调度 只有工作子协程接收调度器调度 调度协程与线程主协程不接受调度
